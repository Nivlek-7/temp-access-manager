...
class ControllerSecurityTest {
    ...
    void cadastroELoginSaoPublicos() throws Exception {
        when(authService.login("usuario@example.com", "senha"))
                .thenReturn(new AuthResponseDto("token-jwt", Role.USER));

        mockMvc.perform(post("/api/auth/registrar")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new Registro("Usuário", "usuario@example.com", "senha"))))
    ...
    void administradorAprovaERejeitaUsuarios() throws Exception {
        ...
        mockMvc.perform(post("/api/usuario/aprovar/1")
                        .header("Authorization", "Bearer admin-token"))
                .andExpect(status().isOk())
                .andExpect(content().string("Aprovado"));

        mockMvc.perform(post("/api/usuario/rejeitar/2")
                        .header("Authorization", "Bearer admin-token"))
                .andExpect(status().isOk())
    ...
    void emailDuplicadoRetorna409() throws Exception {
        doThrow(new EmailJaCadastradoException()).when(usuarioService)
    ...
    private void autenticar(String token, String email, String role) {
        ...
        when(userDetailsService.loadUserByUsername(email))
                .thenReturn(User.withUsername(email).password("senha").roles(role).build());
    }
